...
public class DashboardResponseDTO {
    ...
    private List<NoticeDTO> notices;                   // 공지 목록
    private ActivityCalendarDTO seasonActivity;        // 이번 시즌 활동(잔디)

    public static DashboardResponseDTO of(DashboardDTO dashboard) {
        return new DashboardResponseDTO(
                dashboard.getDecay(),
                dashboard.getTodayPicks(),
                dashboard.getNearbyRanking(),
                dashboard.getSeason(),
                dashboard.getWeekly(),
                dashboard.getNotices(),
                dashboard.getSeasonActivity()
        );
    }
}
